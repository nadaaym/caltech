...
myOptimizer = SGD(lr=0.001, momentum=0.8)  # gradient decent with low learning rate
# myOptimizer = Adam(lr=0.0001)      # adam with low learning rate

# create the model
#myModel = Model(inputs=[preTrainedModel.input], outputs=[outputLayer])
myModel=load_model('/home/nadaaym/caltech1/caltech/run1/model')
...
checkpointCallback = ModelCheckpoint(filepath="./weights1.hdf5", monitor="val_acc", verbose=1, save_best_only=True)
...
history = myModel.fit_generator(trainingDataGenerator,
                                steps_per_epoch=numberOfTrainingBatches, epochs=40, validation_data=validationDataGenerator,
